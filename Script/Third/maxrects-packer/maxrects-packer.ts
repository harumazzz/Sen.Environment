namespace Sen.Script.Third.MaxRectsAlgorithm {
	...
	export class MaxRectsBin<T extends IRectangle = Rectangle> extends Bin<T> {
		...
		public override freeRects: Rectangle[];
		public override rects: T[];
		private verticalExpand: boolean = false;
		private stage: Rectangle;
		private border: number;
		public override options: IOption = {
			smart: true,
			pot: true,
			square: true,
			allowRotation: false,
		...
		constructor(
			public override maxWidth: number = EDGE_MAX_VALUE,
			public override maxHeight: number = EDGE_MAX_VALUE,
		...
		public add(...args: any[]): any {
			...
			if (args.length === 1) {
				...
				let tag =
					rect.data && rect.data.tag ? rect.data.tag : rect.tag ? rect.tag : undefined;
		...
		private expandFreeRects(width: number, height: number) {
			this.freeRects.forEach((freeRect, _) => {
				if (
	...
	export class OversizedElementBin<T extends IRectangle = Rectangle> extends Bin<T> {
		...
		public override maxWidth: number;
		public override maxHeight: number;
		public override options: IOption;
		public override rects: T[] = [];
		public override freeRects: IRectangle[];
	...
	export class MaxRectsPacker<T extends IRectangle = Rectangle> {
		...
		public addArray(rects: T[]) {
			if (!this.options.tag || this.options.exclusiveTag) {
				...
				let targetBin = this.bins.slice(this._currentBinIndex).find((bin, _) => {
					...
					for (let i = currentIdx; i < rects.length; i++) {
						...
						if (tag !== currentTag) {
							...
							currentTag = tag;
							// do addArray()
							this.sort(rects.slice(currentIdx, i), this.options.logic).forEach((r) =>
								bin.add(r),
							);
							currentIdx = i;
