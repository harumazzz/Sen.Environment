namespace Sen.Script.Support.PopCap.Animation.Miscellaenous.GenerateFlashFromMedia {
	...
	export function execute(): void {
		const source = Console.path(
			Kernel.Language.get(
				'script.helper.pvz2.animation.generate_flash_from_media.input_media_path',
			),
			'directory',
		);
		Console.argument(
			Kernel.Language.get(
				'script.helper.pvz2.animation.generate_flash_from_media.enter_animation_name',
			),
		);
		...
		Console.output(
			`${Kernel.Language.get(
				'script.helper.pvz2.animation.generate_flash_from_media.total_image_count',
			)}: ${image_list.length}`,
		);
		...
		for (let e of image_list) {
			...
			Kernel.Image.write(
				`${destination}/library/media/${animation_name}_${image_dimension}.png`,
				image,
			);
		}
		Kernel.Support.PopCap.Animation.ToFlash.process(
			animation,
			destination,
			input_generic,
			has_label,
