namespace Sen.Script.Helper.WWise.AddMusic {
	...
	export function encode_audio(data: InjectionMusicStruct, source: string) {
		...
		for (let media_element of data.media) {
			...
			Support.Wwise.Media.Encode.process_fs(
				`${source}/media/${media_element}.wav`,
				`${source}/.cache/${destination.value}.wem`,
				data.setting.audio_format,
			);
		}
		return media_list;
	}
	...
	export function execute() {
		const source: string = Console.path(
			Kernel.Language.get('script.helper.wwise.add_music.input_music_path'),
			'directory',
		);
		const global_data_source: string = Console.path(
			Kernel.Language.get('script.helper.wwise.add_music.input_global_data_path'),
			'file',
		);
		...
		const global_data_destination = `${temporary_path}/${Kernel.Path.basename(
			global_data_source,
		)}.soundbank`;
		Kernel.Support.WWise.SoundBank.decode_fs(global_data_source, global_data_destination);
		Kernel.Support.WWise.SoundBank.Miscellaneous.add_music(
			source,
