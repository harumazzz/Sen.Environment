namespace Sen.Script.Helper.WWise.ApplyIDHashTable {
	export function execute() {
		const source: string = Console.path(
			Kernel.Language.get(
				'script.helper.wwise.apply_id_hash_table.input_soundbank_data_path',
			),
			'file',
		);
		const table_source: string = Console.path(
			Kernel.Language.get('script.helper.wwise.apply_id_hash_table.input_id_table_json_path'),
			'file',
		);
		...
		Kernel.JSON.serialize_fs(
			`${Kernel.Path.except_extension(source)}.hash.json`,
			data,
			1,
			true,
		);
