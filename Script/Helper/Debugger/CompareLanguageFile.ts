namespace Sen.Script.Helper.Debugger.CompareLanguageFile {
	...
	export function execute(): void {
		const source: string = Console.path(
			Kernel.Language.get('script.compare_language.input_original_language_file'),
			'file',
		);
		const destination: string = Console.path(
			Kernel.Language.get('script.compare_language.input_modified_language_file'),
			'file',
		);
		const missing_key = process(
			Kernel.JSON.deserialize_fs<LanguageFile>(source),
