namespace Sen.Script.Helper.PopCap.Animation.FlashToAnimation {
    const k_xmlns_xsi: Support.PopCap.Animation.DOM.xmlns_xsi = "http://www.w3.org/2001/XMLSchema-instance";
    ...
    export type T = Support.PopCap.Animation.DOM.SpriteDomLayer | Support.PopCap.Animation.DOM.SpriteDomFrame;

    export function push_to_array(data: T): Support.PopCap.Animation.DOM.SpriteDomLayer | Support.PopCap.Animation.DOM.SpriteDomFrame | Array<T> {
    ...
    export function exchange_image(container: Container, layer: Support.PopCap.Animation.DOM.SpriteDomLayer[] | Kernel.XML.XMLDocument, name: string): string {
        if (!Array.isArray(layer)) {
            layer = push_to_array((layer as any)["DOMSymbolItem"]["timeline"]["DOMTimeline"]["layers"]["DOMLayer"]) as Support.PopCap.Animation.DOM.SpriteDomLayer[];
        }
    ...
    export function exchange_sprite(source: string, container: Container, sprite: Kernel.XML.XMLDocument, sprite_name: string): string {
        const is_sprite = sprite_name !== "main_sprite";
        const dom = is_sprite ? (sprite as any)["DOMSymbolItem"] : (sprite as any)["DOMDocument"];
        const dom_timelines = is_sprite ? dom["timeline"]["DOMTimeline"] : (dom["timelines"]["DOMTimeline"] as any);
        const layer_list = push_to_array(dom_timelines["layers"]["DOMLayer"]) as Support.PopCap.Animation.DOM.SpriteDomLayer[];
        ...
        while (layer_index >= end_layer) {
            ...
            const e_dom_frames = push_to_array(e_layer.frames.DOMFrame as T) as Support.PopCap.Animation.DOM.SpriteDomFrame[];
            ...
            for (let i = 0; i < e_dom_frames.length; i++) {
                ...
                if (is_contain_layer && e_dom_frame.elements === null) {
                    ...
                    if (is_contain_layer) {
                        const current_duration = parseInt(e_dom_frame["@attributes"].index) + parseInt(e_dom_frame["@attributes"].duration ?? "0");
                        container.check_duration(current_duration);
                        const e_dom_symbol = (e_dom_frame.elements as Support.PopCap.Animation.DOM.ElementsDOMSymbolInstance)["DOMSymbolInstance"];
                        if (e_dom_symbol !== undefined && e_dom_symbol["@attributes"]["libraryItemName"] !== undefined) {
                            ...
                            if (!container.has_sprite(item_name)) {
                                const symbol: Kernel.XML.XMLDocument = Kernel.XML.deserialize_fs(`${source}/library/${item_name}.xml`);
                                let name_sprite = (symbol["DOMSymbolItem"] as any)["@attributes"]["linkageClassName"] as string;
                                if (name_sprite !== undefined) {
                                    if (name_sprite.startsWith("holder_custom")) {
                                        name_sprite = name_sprite.replace("holder_custom", "_custom");
                                    }
                                    library_name = exchange_sprite(source, container, symbol, name_sprite.toLowerCase());
                                } else {
                                    const symbol_name = (symbol["DOMSymbolItem"] as any)["@attributes"]["name"] as string;
                            ...
                            const library_name = exchange_image(container, [e_layer], `${sprite_name}.${layer_index}.${i}`);
                            sprite_layer.push({
                                "@attributes": {
                                    name: `${layer_index}`,
                                },
    ...
    export function exchange_document(dest: string, container: Container, data: Kernel.XML.XMLDocument): void {
        ...
        Object.values(container.image_container).forEach((e, i) => {
            ...
            Kernel.FileSystem.write_file(`${dest}/library/image/${e}.xml`, container.symbol_list[i]);
        });
        Object.values(container.sprite_container).forEach((e, i) => {
            if (e !== "main_sprite") {
                ...
                Kernel.FileSystem.write_file(`${dest}/library/sprite/${e}.xml`, container.sprite_list[i]);
            } else {
                includes.push({
                    "@attributes": {
                ...
                Kernel.FileSystem.write_file(`${dest}/library/main_sprite.xml`, container.sprite_list[i]);
            }
        });
        const dom: Support.PopCap.Animation.DOM.DOMDocument = {
            DOMDocument: {
                "@attributes": (data["DOMDocument"] as any)["@attributes"] as Support.PopCap.Animation.DOM.DocumentAttributes,
                folders: {
                    DOMFolderItem: [
                        {
                            "@attributes": {
        ...
        const label_dom_layers = (first_layer.frames as any).DOMFrame as Support.PopCap.Animation.DOM.SpriteDomFrame[];
        ...
        Kernel.FileSystem.write_file(`${dest}/DomDocument.xml`, Sen.Kernel.XML.serialize(dom));
        return;
    }

    export function exchange_script(dest: string, symbol_length: number, animation_name: string, resolution: number): void {
        ...
        for (let i = 0; i < symbol_length; i++) {
            const image = `document.library.editItem("image/image_${i + 1}"); document.selectAll(); convert("image_${i + 1}");`;
            image_list.push(image);
            if (image_list.length == 30) {
                if (script_count == 1) {
                    Kernel.FileSystem.write_file(
                        `${dest}/script_${script_count}.jsfl`,
                        script_string.replace("{0}", "var libraryLength = document.library.items.length;").replace("{1}", image_list.join("\n")),
                    );
                } else {
                    Kernel.FileSystem.write_file(`${dest}/script_${script_count}.jsfl`, script_string.replace("{1}", image_list.join("\n")));
                }
                image_list = [];
                ++script_count;
            }
    ...
    export function process(source: string, dest: string, animation_name: string, resolution: number): void {
        Kernel.FileSystem.create_directory(`${dest}/library`);
        const dom_document: Kernel.XML.XMLDocument = Kernel.XML.deserialize_fs(Kernel.Path.join(source, "DomDocument.xml"));
        const container: Container = new Container();
        exchange_sprite(source, container, dom_document, "main_sprite");
        exchange_document(dest, container, dom_document);
        Sen.Kernel.FileSystem.write_file(`${dest}/main.xfl`, "PROXY-CS5");
    ...
    export function execute(): void {
        ...
        process(`${source}_unpack`, `${dest}/${type}${animation_name}.package/resource/${path}`, animation_name, resolution);
        Kernel.JSON.serialize_fs(`${dest}/${type}${animation_name}.package/data.json`, data, 1, true);
        return;
    }
}
