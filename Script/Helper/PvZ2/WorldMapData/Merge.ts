namespace Sen.Script.Helper.PvZ2.WorldMapData.Merge {
	...
	export function process(source: string): Split.WorldMapData {
		...
		const definition: Split.Definition = Kernel.JSON.deserialize_fs<Split.Definition>(
			`${source}/map_list.json`,
		);
		for (const map_name of Object.keys(definition.objects)) {
			const data = Kernel.JSON.deserialize_fs<Split.Object>(
				`${source}/worldmap/${map_name}.json`,
			);
	...
	export function execute(): void {
		const source: string = Console.path(
			Kernel.Language.get('script.merge_worldmap_data.input_source_directory'),
			'directory',
		);
