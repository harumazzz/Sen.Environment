namespace Sen.Script.Helper.PvZ2.Permanent.OrgainizeResources {
	...
	export function process(source: string, destination: string): void {
		...
		resources.groups.sort((a: Resource_Structure_Template, b: Resource_Structure_Template) =>
	...
	export function execute(): void {
		const source: string = Console.path(
			Kernel.Language.get('script.orgainize_resources.source_file'),
			'file',
		);
		return process(source, source.replace(/((\.json))?$/i, '.orgainize.json'));
	}
}
