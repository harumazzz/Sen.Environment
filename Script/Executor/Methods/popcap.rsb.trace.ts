namespace Sen.Script.Executor.Methods.PopCap.RSB.Trace {
	...
	export function backup_content(source: string, destination: string) {
		Kernel.FileSystem.Operation.copy(
			Kernel.Path.normalize(source),
			Kernel.Path.normalize(destination),
		);
	}

	export function forward(): void {
		return push_as_module<
			Methods.PopCap.RSB.Trace.Argument,
			Methods.PopCap.RSB.Trace.BatchArgument,
			Methods.PopCap.RSB.Trace.Configuration
		>({
			id: 'popcap.rsb.trace',
			...
			direct_forward(argument: Argument): void {
				...
				const packages_info: BuildProject.PackagesInfo | null = (
					Kernel.JSON.deserialize_fs(`${argument.source}/data.json`) as any
				).packages_info;
				const count_of = () => {
					const result: Support.Miscellaneous.Custom.ResourceStreamBundle.Configuration.PackagesSetting =
						BuildProject.load_packages(argument.source, packages_info);
					if (packages_info !== null) {
						if (result.json_count !== 0n && packages_info.encode) {
							Console.output(
								`${Kernel.Language.get(
									'popcap.rsb.build_project.total_json_count',
								)}: ${result.json_count}`,
							);
				...
				watcher.on('delete', (e) => {
					...
					Console.obtained(
						`${timestamp}: ${Kernel.Language.get('popcap.rsb.trace.delete')} ${e}`,
					);
					clock.start_safe();
					...
					Kernel.FileSystem.write_file(
						`${repo.directory}/${timestamp}/change.txt`,
						`Delete ${e}`,
					);
					...
					clock.stop_safe();
					Console.send(
						`${Kernel.Language.get('elapsed_time')}: ${clock
							.duration_as_seconds()
							.toFixed(3)}s`,
				...
				watcher.on('update', (e) => {
					const timestamp = current_date();
					Console.obtained(
						`${timestamp.split('-')}: ${Kernel.Language.get(
							'popcap.rsb.trace.update',
						)} ${e}`,
					);
					clock.start_safe();
					const home = `${repo.directory}/${timestamp}`;
				...
				Console.argument(
					format(Kernel.Language.get('popcap.rsb.is_watching'), argument.source),
				);
				watcher.start();
				return;
			},
