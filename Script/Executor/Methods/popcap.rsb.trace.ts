namespace Sen.Script.Executor.Methods.PopCap.RSB.Trace {
    // Generic
    ...
    export function forward(): void {
        Sen.Script.Executor.push_as_module<
            Sen.Script.Executor.Methods.PopCap.RSB.Trace.Argument,
            Sen.Script.Executor.Methods.PopCap.RSB.Trace.BatchArgument,
            Sen.Script.Executor.Methods.PopCap.RSB.Trace.Configuration
        >({
            id: "popcap.rsb.trace",
            configuration_file: Home.query("~/Executor/Configuration/popcap.rsb.trace.json"),
            direct_forward(argument: Argument): void {
                ...
                const packages_info: BuildProject.PackagesInfo | null = (Kernel.JSON.deserialize_fs(`${argument.source}/data.json`) as any).packages_info;
                const count_of = () => {
                    const result: Script.Support.Miscellaneous.Custom.ResourceStreamBundle.Configuration.PackagesSetting = BuildProject.load_packages(argument.source, packages_info);
                    if (packages_info !== null) {
                        if (result.json_count !== 0n && packages_info.encode) {
                            Console.output(`${Kernel.Language.get("popcap.rsb.build_project.total_json_count")}: ${result.json_count}`);
                        ...
                        Console.output(`${Kernel.Language.get("popcap.rsb.build_project.total_rton_count")}: ${result.rton_count}`);
                    } else {
                ...
                Console.warning(Kernel.Language.get("popcap.rsb.trace.copying_root"));
                ...
                watcher.on("delete", (e) => {
                    ...
                    Console.obtained(`${timestamp}: ${Kernel.Language.get("popcap.rsb.trace.delete")} ${e}`);
                    clock.start_safe();
                    ...
                    clock.stop_safe();
                    Console.send(`${Kernel.Language.get("elapsed_time")}: ${clock.duration_as_seconds().toFixed(3)}s`, Definition.Console.Color.GREEN);
                });
                watcher.on("update", (e) => {
                    const timestamp = current_date();
                    Console.obtained(`${timestamp.split("-")}: ${Kernel.Language.get("popcap.rsb.trace.update")} ${e}`);
                    clock.start_safe();
                    const home = `${repo.directory}/${timestamp}`;
                    Kernel.FileSystem.create_directory(home);
