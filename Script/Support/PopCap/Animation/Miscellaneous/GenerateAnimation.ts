namespace Sen.Script.Support.PopCap.Animation.Miscellaenous.GenerateAnimation {
			...
			switch (input_generic) {
				...
				case 2n:
					...
					while (true) {
						Console.argument(
							`${Kernel.Language.get(
								'popcap.animation.miscellaneous.to_apng.input_sprite_to_disable',
							)}`,
						);
						...
						for (const index_result of argument_result) {
							...
							if (sprite_information_map[sprite_list[Number(index_result - 1n)]]) {
								...
								sprite_information_map[sprite_list[Number(index_result - 1n)]] =
									true;
							}
							sprite_to_disable +=
								sprite_to_disable === '' ? `${index_result}` : `, ${index_result}`;
						}
						if (selected_list.length > 0) {
							Console.warning(
								`${Kernel.Language.get(
									'popcap.animation.miscellaneous.to_apng.sprite_already_selected',
								)}: ${selected_list.join(', ')}`,
							);
						}
	...
	export function scale_image(image: Kernel.ImageAdapter.Image, percent: number) {
		if (percent !== 1) {
			...
			const resized_image_data = new DataView(
				new ArrayBuffer(Number(new_width * new_height * 4n)),
			);
			...
			for (let j = 0n; j < new_height; j++) {
				for (let i = 0n; i < new_width; i++) {
					...
					const old_index = Number(
						(old_pixel_height * image.width + old_pixel_width) * 4n,
					);
	...
	export function load_media_source(
		animation: SexyAnimation,
		media_source: string,
		setting: Setting,
	) {
		...
		for (const image of animation.image) {
			media_source_list.push({
				...
				image: scale_image(
					Kernel.Image.open(
						`${media_source}/${setting.image_id ? image.id : image.path}.png`,
					),
					setting.rendering_size.scale,
				),
			});
		}
	...
	export function draw_image(
		canvas: Kernel.Canvas,
		layer: VisualLayer,
		sprite_list: Array<VisualSpriteFrame>,
		media_source: Array<MediaSource>,
		setting: Setting,
	) {
		if (layer.sprite_frame !== null) {
			...
			const resize_matrix = mix_matrix(matrix, [
				setting.rendering_size.scale,
				0,
				0,
				setting.rendering_size.scale,
	...
	export function write_frames(
		visual_sprite_frame: VisualSpriteFrame,
		sprite_list: Array<VisualSpriteFrame>,
		media_source: Array<MediaSource>,
		destination: string,
		setting: Setting,
		...
		for (const frame_index in visual_sprite_frame) {
			...
			const image_path = `${destination}/${setting.frame_name!}_${
	...
	export function exchange_area(
		...
		const rect_image = (layer: VisualLayer) => {
			if (layer.sprite_frame !== null) {
				...
				const after_apply_matrix_to_media_dimension = calculator_dimension(
					media.size,
					resize_matrix,
				);
				...
				rectangle.right = Math.max(
					rectangle.right,
					resize_matrix[4] + after_apply_matrix_to_media_dimension.width,
				);
				rectangle.bottom = Math.max(
					rectangle.bottom,
					resize_matrix[5] + after_apply_matrix_to_media_dimension.height,
				);
			}
		};
		for (const frame_index in visual_sprite_frame) {
	...
	export function process(
		...
		Console.output(
			`${Kernel.Language.get('popcap.animation.to_apng.width')}: ${
				setting.rendering_size.width
			}`,
		);
		Console.output(
			`${Kernel.Language.get('popcap.animation.to_apng.height')}: ${
		...
		const frame_rate =
			setting.apng_setting.frame_rate <= 0n
				? animation.frame_rate
				: setting.apng_setting.frame_rate;
			...
			if (setting.apng_setting.split_label) {
				...
				for (const [label_name, label_info] of Object.entries(definition)) {
					...
					for (let i = label_info.frame_start; i <= label_info.frame_end; i++) {
						label_path_list.push(
							`${destination}/frames/${setting.frame_name}_${i}.png`,
						);
						label_frame_list.push(frame_rate);
					}
					write_apng(
						label_path_list,
						`${destination}/label/${label_name}.apng`,
						label_frame_list,
					);
				...
				write_apng(
					main_animation_path_list,
					`${destination}/animation.apng`,
					main_animation_frame_list,
				);
			} else {
				const main_animation_path_list: Array<string> = [];
				const main_animation_frame_list: Array<bigint> = [];
				for (const frame_index in main_visual_sprite) {
					main_animation_path_list.push(
						`${destination}/frames/${setting.frame_name}_${
							Number(frame_index) + 1
						}.png`,
					);
