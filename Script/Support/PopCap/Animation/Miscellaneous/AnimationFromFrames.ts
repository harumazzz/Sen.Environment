namespace Sen.Script.Support.PopCap.Animation.Miscellaenous.AnimationFromFrames {
	export type DataInfo = GenerateAnimation.DataInfo;

	export type Rectangle = GenerateAnimation.Rectangle;

	export type PostionAdditional = GenerateAnimation.PostionAdditional;
	...
	export function find_non_transparent_bounds(
    ...
    */
	...
	export function exchange_frame(
		...
		const rectangle = find_non_transparent_bounds(
			source_data,
			Number(image.width),
			Number(image.height),
		);
		...
		let pixel_index = 0;
		const image_source_data = new DataView(
			new ArrayBuffer(dimension.width * dimension.height * 4),
		);
	...
	export function process(source: string, destination: string, setting: Setting) {
		...
		const atlases: Atlas.Structure.Definition = {
			method: 'path',
			expand_path: 'array',
		...
		for (let i = 0; i < source_list.length; i++) {
			...
			Kernel.ImageView.write_fs(
				...
				Kernel.ImageView.instance(
					BigInt(source.size.width),
					BigInt(source.size.height),
					source.data,
				),
			);
		}
		animation.main_sprite.work_area.duration = BigInt(animation.main_sprite.frame.length);
		Kernel.JSON.serialize_fs(`${destination}/${animation_name}.pam.json`, animation);
		Kernel.JSON.serialize_fs(`${destination}/${animation_name}.sprite/atlas.json`, atlases);
		const atlas_size: Atlas.Pack.Detail.SizeRange<number> = {
			width:
				setting.atlas_dimension.width <= 0n
					? Number(k_default_atlas_size)
					: Number(setting.atlas_dimension.width),
			height:
				setting.atlas_dimension.height <= 0n
					? Number(k_default_atlas_size)
					: Number(setting.atlas_dimension.height),
			padding: 1,
		};
		const detail: Atlas.Pack.Detail.Data = {
			smart: true,
			pot: false,
			allowRotation: false,
			square: false,
