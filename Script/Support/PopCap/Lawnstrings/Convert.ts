namespace Sen.Script.Support.PopCap.LawnStrings.Convert {
	...
	export function map_to_text(source: JsonMap): string {
		...
		keys.forEach((key: string) => {
			...
			text = text.concat('\n');
			text = text.concat(
				`${source.objects[0].objdata.LocStringValues[key].replace(/\r/g, ``)}`,
			);
			text = text.concat('\n');
			text = text.concat('\n');
		});
	...
	export function array_to_text(information: JsonArray): string {
		...
		for (let i: number = 0; i < information.objects[0].objdata.LocStringValues.length; ++i) {
			...
			text = text.concat('\n');
			text = text.concat(
				`${information.objects[0].objdata.LocStringValues[i + 1].replace(/\r/g, ``)}`,
			);
			text = text.concat('\n');
			text = text.concat('\n');
			++i;
	...
	export function array_to_map(information: JsonArray): JsonMap {
		...
		for (let i: number = 0; i < information.objects[0].objdata.LocStringValues.length; ++i) {
			json_map.objects[0].objdata.LocStringValues[
				information.objects[0].objdata.LocStringValues[i]
			] = information.objects[0].objdata.LocStringValues[i + 1];
			++i;
		}
		return json_map;
	}
	...
	export function map_to_array(information: JsonMap): JsonArray {
		...
		for (let key of keys) {
			...
			json_array.objects[0].objdata.LocStringValues.push(
				information.objects[0].objdata.LocStringValues[key],
			);
		}
		return json_array;
	}
	...
	export function process<
		T extends string | JsonArray | JsonMap,
		Value extends string | JsonArray | JsonMap,
	>(source: T, source_type: Conversion, destination_type: Conversion): Value {
		assert(
			source_type !== destination_type,
			Kernel.Language.get('popcap.lawnstrings.convert.invalid_conversion_type'),
		);
		switch (source_type) {
			case 'cn-text':
			case 'text':
	...
	export function process_fs(
		...
		switch (source_type) {
			...
			default:
				assert(
					false,
					format(
						Kernel.Language.get('popcap.lawnstrings.convert.unsupported_type'),
						source_type,
					),
				);
		}
		...
		switch (destination_type) {
			...
			case 'map':
			case 'array':
				Kernel.JSON.serialize_fs(destination, destination_data);
				break;
