namespace Sen.Script.Support.PopCap.Atlas.MultiResolution {
	...
	export function generalization(
		...
		const definition: Definition = generator(
			Kernel.JSON.deserialize_fs<Definition>(`${source}/atlas.json`),
			new RegExp(`${before}`, 'i'),
			`${after}` as Resolution,
		);
	...
	export type XCallback = (
		source: string,
		size: Pack.Detail.SizeRange<number>,
		detail: Pack.Detail.Data,
		destination: string,
	) => void;
	...
	export function process(
		category: Array<MultiResolution.Category>,
		size: Pack.Detail.SizeRange<number>,
		detail: Pack.Detail.Data,
		callback: XCallback,
	): void {
