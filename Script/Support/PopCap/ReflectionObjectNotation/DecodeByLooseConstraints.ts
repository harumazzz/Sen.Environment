namespace Sen.Script.Support.PopCap.ReflectionObjectNotation.DecodeByLooseConstraints {
	...
	export function read_utf8_character(stream: DataStreamView, size: { value: bigint }): bigint {
		...
		if (current < 128n) {
			...
			assert(
				false,
				`${Kernel.Language.get('popcap.rton.force_decode.position')}: ${
					stream.read_position
				} : ${Kernel.Language.get(
					'popcap.rton.force_decode.invalid_utf8_first_character',
				)}`,
			);
			...
			if ((current & 192n) != 128n) {
				assert(
					...
					`${Kernel.Language.get('popcap.rton.force_decode.position')}: ${
						...
						'popcap.rton.force_decode.invalid_utf8_extra_character',
					)}`,
				);
			}
		...
		if (size !== actual_size) {
			Console.warning(
				`${Kernel.Language.get('popcap.rton.force_decode.position')}: ${
					...
					'popcap.rton.force_decode.invalid_string_size',
				)}: ${Kernel.Language.get('got')} ${size} ${Kernel.Language.get(
					'popcap.rton.force_decode.but_actual',
				)} ${actual_size}`,
			);
		}
		return str;
	...
	export function catch_error(
		...
		assert(
			false,
			`${Kernel.Language.get(
				'popcap.rton.force_decode.invalid_bytecode',
			)}: ${bytecode.toString(16)} | ${Kernel.Language.get(
				'popcap.rton.force_decode.position',
			)}: ${stream.read_position} ${Kernel.Language.get('in')} ${function_name}`,
		);
	}
