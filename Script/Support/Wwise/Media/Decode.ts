namespace Sen.Script.Support.Wwise.Media.Decode {
	export function process(source: string, destination: string) {
		...
		assert(
			...
			Kernel.Language.get('wwise.media.decode.unsupported_operating_system'),
		);
		const vgmstream_program_third_path = Home.query('~/../thirdapp/vgmstream');
		const vgmstream_program_file_path = `${vgmstream_program_third_path}/vgmstream-cli${
			operating_system === 'Windows' ? '.exe' : ''
		}`;
		assert(
			Kernel.FileSystem.is_file(vgmstream_program_file_path),
			Kernel.Language.get('wwise.media.decode.cannot_find_third'),
		);
		...
		assert(
			format !== null,
			Script.format(
				`${Kernel.Language.get('wwise.media.decode.unknown_audio_format')}`,
				encoding,
			),
		);
		Console.display(
