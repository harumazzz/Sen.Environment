namespace Sen.Script.Support.Wwise.Media.Common {
	...
	export function search_path(name: string): string | null {
		let result: null | string = null;
		const item_delimiter = Kernel.OperatingSystem.current() === 'Windows' ? ';' : ':';
		assert(
			Kernel.Process.is_exists_in_path_environment('PATH'),
			Kernel.Language.get('environment_path_not_found'),
		);
		const path_environment_list =
			Kernel.Process.get_path_environment('PATH').split(item_delimiter);
		const path_extension_list = [''];
		if (Kernel.OperatingSystem.current() === 'Windows') {
			assert(
				Kernel.Process.is_exists_in_path_environment('PATHEXT'),
				Kernel.Language.get('environment_pathext_not_found'),
			);
			const path_extension_environment = Kernel.Process.get_path_environment('PATHEXT');
			path_extension_list.push(
				...path_extension_environment
		...
		for (let path of path_environment_list) {
			...
			let path_extension = path_extension_list.find((value) =>
				Kernel.FileSystem.is_file(`${path_base}${value}`),
			);
