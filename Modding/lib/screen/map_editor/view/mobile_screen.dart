                ...
                child: SizedBox(
                    ...
                    child: Card.outlined(
                      ...
                      child: BlocBuilder<SettingBloc, SettingState>(
                          buildWhen: (prev, state) => prev.showTopScreenShortcut != state.showTopScreenShortcut,
                          builder: (context, settingState) {
                            return Stack(
                              fit: StackFit.passthrough,
                              children: [
                                const MapStageView(),
                                if (state.navigator != NavigationType.option) const ShopWidget(),
                                if (state.navigator != NavigationType.option) const ExtensionWidget(),
                                if (settingState.showTopScreenShortcut) const ScreenShortcut(),
                              ],
                            );
                          }),
                    )),
              ...
              NavigationBar(
                destinations: [
                  ...NavigationType.values.map((e) {
                    ...
                    return NavigationDestination(icon: item.icon, label: item.title, tooltip: item.description);
                  })
                ],
                ...
                onDestinationSelected: (index) {
                  context.read<SectionBloc>().add(
                        SetNavigator(
                          navigator: NavigationType.values[index],
                        ),
                      );
                },
              ),
  ...
  Widget build(BuildContext context) {
    ...
    switch (navigator) {
      case NavigationType.tool:
        {
          return Padding(
              padding: const EdgeInsets.symmetric(horizontal: 4), child: ToolBarView(toolItem: state.toolItem));
        }
