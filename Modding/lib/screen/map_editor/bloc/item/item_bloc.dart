...
class ItemBloc extends Bloc<ItemEvent, ItemState> {
  ...
  (VisualImage, bool) _getVisualImage(int id) {
    ...
    if (items.containsKey(id)) {
      ...
      return (cubit.state.gameResource.commonImage[ImageCommonType.missingArtPiece]!, false);
    }
  }
  ...
  (VisualAnimation, bool) _getVisualAnimation(int id) {
    ...
    if (items.containsKey(id)) {
      ...
      return (cubit.state.gameResource.commonAnimation[AnimationCommonType.missingArtPieceAnimation]!, false);
    }
  }
  ...
  void _updatePieces({
    ...
    const animationScaleRatio = MapConst.scaleResolution / MapConst.animationResolution;
    ...
    for (final entry in sortedEntries) {
      ...
      if (piece.pieceType == PieceType.animation) {
        ...
        final usesRasterizedImagesInAnim = rasterizedInAnimation[piece.imageID] ?? false;
        final posXFactor = -(MapConst.animationSizeCenter * scaleXFactor * animationScaleRatio * 0.5);
        // final scaleYFactor = piece.isArtFlipped ? -piece.scaleY : piece.scaleY;
        final posYFactor = -(MapConst.animationSizeCenter * piece.scaleY * animationScaleRatio * 0.5);
        final piecePosX = (itemStartPositionX + piece.position.x) + additionalParallaxPos;
        ...
        final rotateOrigin = usesRasterizedImagesInAnim ? const Offset(250, 250) : const Offset(195, 195);
        final destScale = usesRasterizedImagesInAnim ? imageScaleRatio : animationScaleRatio;
        ...
        final itemRect = _makeRect(visual.visualSize.width, visual.visualSize.height);
        ...
        final piecePosX = (itemStartPositionX + piece.position.x + posXFactor) + additionalParallaxPos;
        final piecePosY = itemStartPositionY + piece.position.y;
        final selectRect = _makeRect(image.width, image.height, viewPortScale: viewportScale);
        final rotateFactor = piece.isArtFlipped ? piece.rotationAngle.toDouble() : -piece.rotationAngle.toDouble();
        final matrix = Matrix4Transform()
            ...
            .rotateDegrees(rotateFactor, origin: Offset(image.width / 2, image.height / 2))
            .m;
      ...
      if (isParallax) {
        ...
        */
        final parallaxList = parallax < 0 ? parallaxTopList : parallaxBottomList;
      ...
      final property = stageBloc.state.pieceProperty[(layer, parallax)] ?? ItemProperty();
      _updateItemProfile(itemProfile, property.status);
    }
  }

  Rect _makeRect(num width, num height, {num left = 0, top = 0, double viewPortScale = 1}) {
    return Rect.fromLTWH(left.toDouble(), top.toDouble(), width * viewPortScale, height * viewPortScale);
  }
  ...
  void _updateEvents({
    ...
    final missingArtPiece = gameResource.commonImage[ImageCommonType.missingArtPiece]!;
    final missingArtPieceAnimation = gameResource.commonAnimation[AnimationCommonType.missingArtPieceAnimation]!;
    final editorSettingState = settingBloc.state;
    ...
    for (final entry in eventSortedEntries) {
      ...
      switch (event.eventType) {
        ...
        case EventType.doodad:
          {
            ...
            final piecePosX = itemStartPositionX + event.position.x + posXFactor;
            final piecePosY = itemStartPositionY + event.position.y + posYFactor;
            final matrix = Matrix4Transform().scale(imageScaleRatio).translate(x: piecePosX, y: piecePosY).m;
            final selectRect = _makeRect(image.width, image.height, viewPortScale: viewportScale);
            final rect = MatrixUtils.transformRect(matrix, selectRect);
            final isVisible = viewportRect.overlaps(rect);
            if (!isVisible && !onSelectedList.contains(entry.key)) {
        ...
        case EventType.path_node:
          {
            ...
            final posXFactor = -image.width * imageScaleRatio;
            final posYFactor = -image.height * imageScaleRatio;
            final piecePosX = itemStartPositionX + event.position.x + posXFactor + 30;
            final piecePosY = itemStartPositionY + event.position.y + posYFactor + 15;
            final matrix = Matrix4Transform().scale(imageScaleRatio).translate(x: piecePosX, y: piecePosY).m;
            final selectRect = _makeRect(image.width, image.height, viewPortScale: viewportScale);
            final rect = MatrixUtils.transformRect(matrix, selectRect);
            final isVisible = viewportRect.overlaps(rect);
        ...
        case EventType.star_gate:
          {
            ...
            const posXFactor = -(MapConst.animationSizeCenter * animationScaleRatio * 0.5);
            // final scaleYFactor = piece.isArtFlipped ? -piece.scaleY : piece.scaleY;
            const posYFactor = -(MapConst.animationSizeCenter * animationScaleRatio * 0.5);
            final piecePosX = itemStartPositionX + event.position.x + posXFactor + 5;
            final piecePosY = itemStartPositionY + event.position.y + posYFactor - 40;
            final matrix = Matrix4Transform().scale(animationScaleRatio).translate(x: piecePosX, y: piecePosY).m;
            final visual = gameResource.commonAnimation[AnimationCommonType.stargate]!;
            final itemRect = _makeEventRect(EventNodeType.stargate)!;
            final selectRect = _makeRect(itemRect.width, itemRect.height,
                top: itemRect.top, left: itemRect.left, viewPortScale: viewportScale);
            final rect = MatrixUtils.transformRect(matrix, selectRect);
            final isVisible = viewportRect.overlaps(rect);
            if (!isVisible && !onSelectedList.contains(entry.key)) {
              continue;
            ...
            itemProfile.widget = Transform(
                ...
                child: AnimationWidget(
                  labelPlay: const [],
                  forceLabelPlay: mapCompleted ? playLabelEvent.$2.first : playLabelEvent.$1.first,
                  borderRect: itemRect,
                  visual: visual,
                  borderColor: Colors.blue,
        ...
        case EventType.key_gate:
          {
            ...
            final visual =
                resourceBloc.state.resourceAnimation[ResourceAnimationType.keyGate] ?? missingArtPieceAnimation;
            const posXFactor = -(MapConst.animationSizeCenter * animationScaleRatio * 0.5);
            const posYFactor = -(MapConst.animationSizeCenter * animationScaleRatio * 0.5);
            ...
            final image = gameResource.commonImage[ImageCommonType.keygateFlag]!;
            final text = (event.cost ?? 0).toString();
            final flag = isArtFlipped
                ? ImageWidget(
                    image: image,
                    matrix: matrix.multiplied(Matrix4Transform().flipHorizontally().translate(x: 147, y: 143).m),
                    borderColor: Colors.white,
                    borderWidth: 0,
                    filterQuality: filterQuality,
                  )
                : ImageWidget(
                    image: image,
                    matrix: matrix.multiplied(Matrix4Transform().translate(x: 247, y: 143).m),
                    borderColor: Colors.white,
                    borderWidth: 0,
                    filterQuality: filterQuality,
                  );
            ...
            final flagInfoImage = gameResource.commonImage[ImageCommonType.infoIcon]!;
            final costInfo = isFinished
                ? (isArtFlipped
                    ? ImageWidget(
                        image: flagInfoImage,
                        matrix:
                            matrix.multiplied(Matrix4Transform().scale(animationScaleRatio).translate(x: 50, y: 160).m),
                        borderColor: Colors.white,
                        borderWidth: 0,
                        filterQuality: filterQuality,
                      )
                    : ImageWidget(
                        image: flagInfoImage,
                        matrix: matrix
                            .multiplied(Matrix4Transform().scale(animationScaleRatio).translate(x: 290, y: 160).m),
                        borderColor: Colors.white,
                        borderWidth: 0,
                        filterQuality: filterQuality,
                      ))
                : (isArtFlipped
                    ? Transform(
                        alignment: Alignment.topLeft,
                        transform: matrix.multiplied(Matrix4Transform().scale(1.6).translate(x: 60, y: 135).m),
                        child: DisplayText(
                          displayText: text,
                          fontSize: 45,
                          strokeWidth: 3,
                    ...
                    : Transform(
                        alignment: Alignment.topLeft,
                        transform: matrix.multiplied(Matrix4Transform().scale(1.6).translate(x: 300, y: 135).m),
                        child: DisplayText(
                          displayText: text,
                          fontSize: 45,
                          strokeWidth: 3,
        ...
        case EventType.upgrade:
          {
            final image = gameResource.upgrade[event.dataString] ?? missingArtPiece;
                    ...
                    children: [
                      Transform(
                          ...
                          transform: Matrix4Transform()
                              ...
                              .m,
                          child: AnimationWidget(
                            visual: gameResource.commonAnimation[AnimationCommonType.collectedUpgradeEffect]!,
                            labelPlay: const ['main'],
                            borderWidth: 0,
                            filterQuality: filterQuality,
                            playSingleFrame: playSingleFrame,
        ...
        case EventType.plant:
          {
            ...
            if (editorSettingState.mapCompleted) {
              ...
              // final scaleYFactor = piece.isArtFlipped ? -piece.scaleY : piece.scaleY;
              const posYFactor = -(MapConst.animationSizeCenter * animationScaleRatio * 0.5);
              final piecePosX = itemStartPositionX + event.position.x + posXFactor - 1;
              final piecePosY = itemStartPositionY + event.position.y + posYFactor - 17;
              final matrix = Matrix4Transform().scale(animationScaleRatio).translate(x: piecePosX, y: piecePosY).m;
              final visual =
                  gameResource.plant[plantType] ?? gameResource.commonAnimation[AnimationCommonType.readyPlant]!;
              ...
              itemProfile.widget = Stack(fit: StackFit.passthrough, children: [
                ImageWidget(
                  ...
                  matrix: matrix.multiplied(Matrix4Transform().scaleBy(x: 0.4, y: 0.5).translate(x: 150, y: 215).m),
                  borderColor: Colors.white,
                  borderWidth: 0,
                  filterQuality: filterQuality,
                ),
              ...
              final plantImage =
                  gameResource.packet[plantType] ?? gameResource.commonImage[ImageCommonType.readyPacket]!;

              final seedBankImage = gameResource.seedBank[plantConfig[plantType]] ??
              ...
              final matrix = Matrix4Transform().scale(imageScaleRatio * 0.75).translate(x: piecePosX, y: piecePosY).m;
              final selectRect = _makeRect(seedBankImage.width, seedBankImage.height, viewPortScale: viewportScale);
              final rect = MatrixUtils.transformRect(matrix, selectRect);
              final isVisible = viewportRect.overlaps(rect);
              if (!isVisible && !onSelectedList.contains(entry.key)) {
                continue;
              ...
              itemProfile.matrix = matrix;
              itemProfile.itemRect = _makeRect(seedBankImage.width, seedBankImage.height);
              itemProfile.selectRect = rect;
              itemProfile.widget = Stack(
                fit: StackFit.passthrough,
                children: [
                  ImageWidget(
                    image: gameResource.commonImage[ImageCommonType.sprout]!,
                    matrix: matrix.multiplied(Matrix4Transform().scaleBy(x: 0.65, y: 0.8).translate(x: 40, y: 127).m),
                    borderColor: Colors.white,
                    borderWidth: 0,
                    filterQuality: filterQuality,
                  ),
        ...
        case EventType.giftbox:
          {
            ...
            final piecePosY = itemStartPositionY + event.position.y + posYFactor;
            final matrix = Matrix4Transform().scale(animationScaleRatio).translate(x: piecePosX, y: piecePosY).m;
            final visual = gameResource.commonAnimation[AnimationCommonType.giftBox]!;
        ...
        case EventType.pinata:
          {
            ...
            final image = editorSettingState.mapCompleted
                ...
                : resourceImage[ResourceImageType.pinataSpine] ?? gameResource.commonImage[ImageCommonType.freePinata]!;
            final posXFactor = -image.width * 0.5 * imageScaleRatio;
        ...
        case EventType.level:
          {
            ...
            const posYFactor = -(MapConst.animationSizeCenter * animationScaleRatio * 0.5);
            final piecePosX = itemStartPositionX + event.position.x + posXFactor;
            final piecePosY = itemStartPositionY + event.position.y + posYFactor - 7;
            ...
            switch (levelNodeType) {
              case LevelNodeType.normal:
                {
                  final visual = gameResource.commonAnimation[AnimationCommonType.levelNode]!;
                    ...
                    children: [
                      ...
                      Transform(
                          ...
                          transform: matrix.multiplied(Matrix4Transform()
                              .translate(x: (visual.visualSize.width / 2) - textLength, y: visual.visualSize.height / 4)
                              .m),
                          child: DisplayText(
                            displayText: text,
                            fontSize: 65,
              ...
              case LevelNodeType.minigame:
                {
                  final visual = gameResource.commonAnimation[AnimationCommonType.levelNodeMinigame]!;
              ...
              case LevelNodeType.miniboss:
                {
                  final visual = gameResource.commonAnimation[AnimationCommonType.levelNodeGargantuar]!;
                  ...
                  final text = event.displayText ?? '';
                  final textLength = text.length * 12;
                  final playLabelEvent = eventAnimationLabel[EventNodeType.miniboss]!;
                  itemProfile.widget = Stack(
                    fit: StackFit.passthrough,
                    children: [
                      Transform(
              ...
              case LevelNodeType.nonfinalboss:
                {
                  ...
                  itemProfile.itemRect = itemRect;
                  itemProfile.selectRect = rect;
                  final playLabelEvent = eventAnimationLabel[EventNodeType.nonfinalboss]!;
                  itemProfile.widget = Transform(
                      alignment: Alignment.topLeft,
                      transform: matrix,
                      child: AnimationWidget(
              ...
              case LevelNodeType.boss:
                {
                  final visualHologram = gameResource.commonAnimation[AnimationCommonType.zombossNodeHologram]!;
                  final visualTop = gameResource.commonAnimation[AnimationCommonType.levelNodeGargantuar]!;
                  final visual = resourceBloc.state.resourceAnimation[ResourceAnimationType.zombossNode] ??
                  ...
                  matrix.scale(MapConst.animationResolution / MapConst.imageResolution);
                  matrix.translate(50.0, 40.0);
                  itemProfile.matrix = matrixTop;
                  itemProfile.itemRect = _makeRect(visualTop.visualSize.width, visualTop.visualSize.height);
                  itemProfile.selectRect = rect;
                  final playLabelZombossNode = eventAnimationLabel[EventNodeType.boss]!;
                  final playLabelHologram = eventAnimationLabel[EventNodeType.hologramBoss]!;
                  final playLabelEvent = eventAnimationLabel[EventNodeType.miniboss]!;
                  itemProfile.widget = Stack(
                    fit: StackFit.passthrough,
                    children: [
                      Transform(
                          alignment: Alignment.topLeft,
                          transform: matrixTop.multiplied(Matrix4Transform().translate(y: -205).m),
                          child: AnimationWidget(
                            labelPlay: mapCompleted ? playLabelHologram.$2 : playLabelHologram.$1,
                  ...
                  zombossNodeProfile.widget = Transform(
                    ...
                    transform: matrix,
                    child: AnimationWidget(
                      labelPlay: mapCompleted ? playLabelZombossNode.$2 : playLabelZombossNode.$1,
                      visual: visual,
                      borderColor: Colors.blue,
                      borderWidth: editorSettingState.eventBorder ? 2.0 : 0,
                      filterQuality: filterQuality,
              ...
              case LevelNodeType.danger:
                {
                  final visual = resourceBloc.state.resourceAnimation[ResourceAnimationType.dangerNode] ??
                      missingArtPieceAnimation;
                  final selectRect = _makeRect(
                    visual.visualSize.width,
                    visual.visualSize.height,
                  ...
                  itemProfile.matrix = matrix;
                  itemProfile.itemRect = _makeRect(visual.visualSize.width, visual.visualSize.height);
                  itemProfile.selectRect = rect;
                  final text = event.displayText ?? '';
                  final textLength = text.length * 8;
                  final mapCompleted = editorSettingState.mapCompleted;
                  final dangerLevelImage =
                      resourceBloc.state.resourceImage[ResourceImageType.dangerLevel] ?? missingArtPiece;
                  final playLabelEvent = eventAnimationLabel[EventNodeType.danger]!;
                  itemProfile.widget = Stack(
                    fit: StackFit.passthrough,
                    children: [
                      Transform(
                      ...
                      if (mapCompleted)
                        ImageWidget(
                          ...
                          matrix: matrix.multiplied(Matrix4Transform()
                              .scale(MapConst.animationResolution / MapConst.imageResolution)
                      ...
                      if (mapCompleted)
                        Transform(
                            ...
                            transform: matrix.multiplied(Matrix4Transform()
                                .translate(
                                    x: (visual.visualSize.width / 2) - textLength, y: visual.visualSize.height / 8)
                                .m),
                            child: DisplayText(
                              displayText: text,
                              fontSize: 45,
    ...
    if (settingBloc.state.mapPath) {
      ...
      final mapPathVisual = gameResource.commonAnimation[AnimationCommonType.mapPath]!;
      ...
      for (final e in eventList.entries) {
        ...
        final targetKey =
            eventList.keys.firstWhere((e) => parentEvent == events[e]!.name, orElse: () => 'null_parent_key');
        if (events[targetKey] != null && parentEvent == events[targetKey]!.name) {
          ...
          final posXFactor = -(MapConst.animationSizeCenter * scaleXFactor * animationScaleRatio * 0.5);
          const posYFactor = -(MapConst.animationSizeCenter * animationScaleRatio * 0.5);
          final piecePosX = itemStartPositionX + startEvent.position.x + (deltaX / 2) + 2 + posXFactor;
          final piecePosY = itemStartPositionY + startEvent.position.y + (deltaY / 2) + posYFactor;
  ...
  void _storeUpdate(ItemStoreUpdated event, Emitter<ItemState> emit) {
    ...
    final itemStartPositionX = (-stageState.boundingRect.x + startPositionX).toDouble();
    final itemStartPositionY = (-stageState.boundingRect.y + startPositionY).toDouble();
    ...
    final cameraPosX = -(controllerTransform[12] / viewportScale + startPositionX);
