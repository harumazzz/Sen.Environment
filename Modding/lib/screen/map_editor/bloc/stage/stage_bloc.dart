...
class StageBloc extends Bloc<StageEvent, StageState> {
  ...
  void _changeWorldResource(
    ...
    if (event.layerBloc.isEmptyNode()) {
      ...
      historyBloc.add(InitailizeCapture(
  ...
  Position _getScreenItemPosition() {
    ...
    final currentPosX =
        -(controllerTransform[12] / viewportScale) + state.boundingRect.x;
    final currentPosY =
        -(controllerTransform[13] / viewportScale) + state.boundingRect.y;
  ...
  void _addEventItem(AddEventItemEvent event, Emitter<StageState> emit) {
    final position = _getScreenItemPosition();
      ...
      case EventNodeType.plant:
        {
          ...
          eventItem.dataString = configModel.resource.plant.keys.firstOrNull;
          break;
        }
  ...
  ActionService<ActionModelType> _mapInitializeService(
    ...
    return ActionService<ActionModelType>(
        ...
        data: {
          ActionModelType.stage: stageState,
        },
        change: (data) {
