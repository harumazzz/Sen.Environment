...
class ScreenShortcut extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final disableColor = Theme.of(context).brightness == Brightness.light ? colorScheme.inverseSurface : colorScheme.outlineVariant;
    return Padding(
        ...
        child: Column(
          children: [
                    ...
                    return Row(
                      children: [
                        ShortcutItem(
                          ...
                          color: state.captureManager.canUndo()
                              ? selectedColor
                              : disableColor,
                          onTap: state.captureManager.canUndo()
                              ? () {
                        ...
                        ShortcutItem(
                            ...
                            color: state.captureManager.canRedo()
                                ? selectedColor
                                : disableColor,
                            onTap: state.captureManager.canRedo()
                                ? () {
            ...
            BlocBuilder<ToolBarBloc, ToolBarState>(builder: (context, state) {
              return Row(
                children: [
                  ShortcutItem(
                      iconData: Symbols.pan_tool,
                      color: state.toolStatus[ToolType.panTool]! ? selectedColor : buttonColor,
                      onTap: () {
                        context.read<ToolBarBloc>().add(const ToolToogled(type: ToolType.panTool));
                      }),
                  ShortcutItem(
                      iconData: Symbols.gesture_select,
                      color: state.toolStatus[ToolType.rectangleTool]! ? selectedColor : buttonColor,
                      onTap: () {
                        context.read<ToolBarBloc>().add(const ToolToogled(type: ToolType.rectangleTool));
                      })
                ],
              );
            })
