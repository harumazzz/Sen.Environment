...
class AnimationWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (spriteFrame.length == 1) {
      ...
      final labelInfo = visual.labelInfo[forceLabelPlay ?? visual.labelPlay] ??
      ...
      child = BlocBuilder<TickerBloc, TickerState>(
        builder: (context, state) {
          ...
          if (forceLabelPlay == null &&
            ...
            visual.labelPlay =
                labelPlay.elementAt(Random().nextInt(labelPlay.length));
          }
    ...
    if (borderWidth > 0) {
      child = CustomPaint(
          painter: BorderPainer(
              itemRect: borderRect ??
                  Rect.fromLTWH(
                      0, 0, visual.visualSize.width, visual.visualSize.height),
