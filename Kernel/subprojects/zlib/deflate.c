...
int32_t Z_EXPORT zng_deflateSetParams(zng_stream *strm, zng_deflate_param_value *params, size_t count) {
    ...
    /* Initialize the statuses. #1#
    for (i = 0; i < count; i++)
        params[i].status = Z_OK;

    /* Check whether the stream state is consistent. #1#
    if (deflateStateCheck(strm))
        return Z_STREAM_ERROR;
    s = strm->state;

    /* Check buffer sizes and detect duplicates. #1#
    for (i = 0; i < count; i++) {
    ...
    /* Exit early if small buffers or duplicates are detected. #1#
    if (buf_error)
        return Z_BUF_ERROR;

    /* Apply changes, remember if there were errors. #1#
    ...
    /* Report version errors only if there are no real errors. #1#
