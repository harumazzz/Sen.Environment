	...
	public:
		...
		) -> Image<int>
		{
			...
			for (auto y : Range<int>(height))
			{
				...
				{
					auto temp_pixel = stream.readUint16();
			...
			int height) -> Image<int>
		{
			...
			for (auto i : Range(image_square)) {
				...
				data[i * 4 + 1] = destination[i * 3 + 1];
				data[i * 4 + 2] = destination[i * 3 + 2];
				data[i * 4 + 3] = index_table[read_bits<int>(bit_depth, bitPostion, buffer, stream)];
			}
			return Image<int>{0, 0, width, height, data};
		}
			...
			int height) -> Image<int>
		{
			...
			for (auto i : Range<int>(width * height)) {
				image_data[i * 4 + 3] = stream.readUint8();
			}
