...
namespace Sen::Kernel::Support::PopCap::ResourceStreamGroup {
    ...
    private:
        static auto exchange_packet_resource(ReadMemoryStream &stream, ItemResource &value, const std::string_view& path) -> void {
        ...
        static auto compiled_map_decode(ReadMemoryStream &stream, List<ItemResource> &value_list) -> void {
            ...
            while (stream.current_position() < resource_information_end) {
                ...
                auto item_resource = ItemResource{};
                exchange_packet_resource(stream, item_resource, string_stream.view());
                value_list.append(item_resource);
            }
        }

        template<auto general_compressed, auto texture_compressed>
        static auto decode_packet_data(ReadMemoryStream &stream, Uint8Array &general, Uint8Array &texture) -> void {
            static_assert(general_compressed == true || general_compressed == false, "general_compressed should be true or false");
            static_assert(texture_compressed == true || texture_compressed == false, "texture_compressed should be true or false");
            ...
            if (const auto general_resource_data_section_size_original = stream.u32(); general_resource_data_section_size_original != k_none_size) {
                if constexpr (general_compressed) {
                    ...
                    auto compressed_data = stream.get_bytes(general_resource_data_section_offset, general_resource_data_section_size);
                    Compression::Zlib::Uncompress::process(compressed_data, general);
                } else {
                    auto uncompressed_data = stream.get_bytes(general_resource_data_section_offset, general_resource_data_section_size_original);
            ...
            if (const auto texture_resource_data_section_size_original = stream.u32(); texture_resource_data_section_size_original != k_none_size) {
                if constexpr (texture_compressed) {
                    ...
                    auto compressed_data = stream.get_bytes(texture_resource_data_section_offset, texture_resource_data_section_size);
                    Compression::Zlib::Uncompress::process(compressed_data, texture);
                } else {
                    auto uncompressed_data = stream.get_bytes(texture_resource_data_section_offset, texture_resource_data_section_size_original);
                    texture.assign(uncompressed_data);
                }
            }
        }

        template <is_class ResourceClass>
