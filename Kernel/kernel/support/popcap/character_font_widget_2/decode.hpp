...
namespace Sen::Kernel::Support::PopCap::CharacterFontWidget2
{
    ...
    private:
        ...
        static auto exchange_font_kerning(ReadMemoryStream& stream, FontKerning& model) -> void
        {
            exchange_number_fixed<u16>(stream, model.offset);
            exchange_unicode_fixed<u16>(stream, model.index);
        }

        static auto exchange_font_character(ReadMemoryStream& stream, FontCharacter& model) -> void
        {
            exchange_unicode_fixed<u16>(stream, model.index);
        ...
        static auto exchange_string_32block(ReadMemoryStream& stream, String& string) -> void
        {
            exchange_string_block<u32>(stream, string);
        }

        static auto exchange_font_layer(ReadMemoryStream& stream, FontLayer& model) -> void
        {
        ...
        static auto exchange_font_widget(ReadMemoryStream& stream, FontWidget& model) -> void
        {
            exchange_number_fixed<i32>(stream, model.ascent);
            exchange_number_fixed<i32>(stream, model.ascent_padding);
            exchange_number_fixed<i32>(stream, model.height);
